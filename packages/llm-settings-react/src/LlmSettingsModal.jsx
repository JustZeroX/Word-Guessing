...
import {
  ...
  getProviderOptions,
} from '../../llm-settings-core/src/index'
...
function LlmSettingsModal({
  ...
  const updateProvider = (provider) => {
    setFormState((prev) => applyProviderPresetToSettings(prev, provider))
    setError('')
    setTestStatus('idle')
    setTestMessage('')
    setTestTrace('')
      ...
      <div className="relative w-full max-w-xl rounded-2xl border border-indigo-100 bg-white/95 p-4 shadow-2xl shadow-indigo-100/70 transition-all duration-300 dark:border-slate-700/60 dark:bg-slate-900/90 dark:shadow-none sm:p-6">
        ...
        <div className="grid grid-cols-1 gap-2 sm:flex sm:flex-wrap sm:gap-3">
          ...
          <button
            ...
            onClick={() => setFormState((prev) => applyProviderPresetToSettings(prev, formState.provider))}
          >
